...
Graphite.prototype._updateHostGroups = function(groups, hosts, id){
    var self = this;
    var out = {};

    for (var group_id in groups) {
        if (group_id == '_label') { continue; }
        ...
        if (typeof cur_group == 'string') {
            ...
            var sub = self._updateHostGroups(cur_group, hosts[group_id], full_id);
            out[group_label] = {
                _id: full_id
            };
            for (var key in sub) {
